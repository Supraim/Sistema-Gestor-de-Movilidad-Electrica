...
public class CargaRepositorioImpl implements CargaRepositorio {
    ...
    public List<Carga> verHistorico(Long idCliente, LocalDateTime fechaIni, LocalDateTime fechaFin){
        try {
            return em.createQuery("SELECT c FROM Carga c WHERE c.cliente.id = :idCliente AND c.horaInicio >= :fechaIni AND c.horaFin <= :fechaFin", Carga.class)
    ...
    public Carga verCargaActual (Long idCliente){
        try {
            return em.createQuery("SELECT c FROM Carga c WHERE c.cliente.id = :idCliente AND c.estado = 'CARGANDO' ", Carga.class)
                    .setParameter("idCliente", idCliente)
                    .getSingleResult();
        }catch (NoResultException e){
            return null;
